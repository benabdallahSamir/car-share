  ...
  if (viewButtons) {
    viewButtons.forEach((button) => {
      button.addEventListener("click", function () {
        ...
        // Récupérer les informations de la plainte depuis la ligne du tableau
        const complaintId =
        ...
        const complaintStatus =
          row.querySelector("td:nth-child(5) .status-badge")?.textContent || "";

        // Mettre à jour TOUS les éléments dans le modal avec les informations de l'utilisateur
        const idElement = document.querySelector(".complaint-id");
        const subjectElement = document.querySelector(".complaint-subject h4");
        const dateElement = document.querySelector(".complaint-date");
        const userNameElements = modal.querySelectorAll(
          ".user-name, .user-info .name"
        );
        const statusElement = document.querySelector(
          ".complaint-status .status-badge"
        );

        if (idElement) idElement.textContent = complaintId;
        if (subjectElement) subjectElement.textContent = complaintSubject;
        if (dateElement) dateElement.textContent = `Créé le ${complaintDate}`;

        // Mettre à jour TOUS les éléments contenant le nom d'utilisateur
        userNameElements.forEach((element) => {
          if (element) element.textContent = complaintUser;
        });

        if (statusElement) {
          statusElement.textContent = complaintStatus;
          statusElement.className = `status-badge ${complaintStatus
            .toLowerCase()
            .replace(" ", "-")}`;
        }

        // Mettre à jour la description de la plainte
        const descriptionElement = document.querySelector(
          ".complaint-description p"
        );
        if (descriptionElement) {
          descriptionElement.textContent = `Description de la plainte ${complaintId} de ${complaintUser}`;
        }

        // Initialiser les messages de la plainte si nécessaire
        if (!complaintMessages[complaintId]) {
          complaintMessages[complaintId] = [
            {
              sender: complaintUser,
              time: `${complaintDate} 10:30`,
              content: `Message initial de ${complaintUser} concernant la plainte ${complaintId}`,
            },
          ];
        }

        // Mettre à jour les messages à partir du stockage
        const messageList = document.querySelector(".message-list");
        if (messageList) {
          messageList.innerHTML = "";
          complaintMessages[complaintId].forEach((message) => {
        ...
        const modalProcessBtn = document.querySelector(".modal-btn.process");
        const modalResolveBtn = document.querySelector(".modal-btn.resolve");

        if (modalProcessBtn) {
          modalProcessBtn.onclick = function () {
            const statusCell = row.querySelector("td:nth-child(5)");
            const actionCell = row.querySelector("td:last-child");
            ...
            // Ajouter un message système pour l'action
            const now = new Date();
            ...
            complaintMessages[complaintId].push({
              sender: "Système",
              time: `${dateStr} ${timeStr}`,
              content: `La plainte a été mise en cours de traitement par Admin.`,
            });
        ...
        if (modalResolveBtn) {
          modalResolveBtn.onclick = function () {
            ...
            complaintMessages[complaintId].push({
              sender: "Système",
              time: `${dateStr} ${timeStr}`,
              content: `La plainte a été résolue par Admin.`,
            });
        ...
        // Mettre à jour le bouton Envoyer pour envoyer un message avec le bon nom d'administrateur
        const sendMessageButton = modal.querySelector(".send-message");
        if (sendMessageButton) {
          // Supprimer les anciens écouteurs d'événements
          const newButton = sendMessageButton.cloneNode(true);
          sendMessageButton.parentNode.replaceChild(
            newButton,
            sendMessageButton
          );
          ...
          newButton.addEventListener("click", function () {
            const messageInput = modal.querySelector(".message-form textarea");
            ...
            if (messageInput && messageInput.value.trim() !== "") {
              ...
              // Ajouter le message à l'objet de stockage
              complaintMessages[complaintId].push({
                sender: "Admin",
                time: `${dateStr} ${timeStr}`,
              ...
              showNotification("Veuillez entrer un message.");
            }
          });
        }
